...
<!-- 
	...
	커넥션풀기법이란? 데이터베이스와 커넥션을 사전에 만들어서 pool에 저장해두고 필요할 때
	마다 풀에서 꺼내서 사용한 후에 다시 pool에 반환하는 기법을 말한다.
	
	커넥션풀의 특징은
	
	1. 사전에 풀에 Connection이 생성되어 있기 때문에 커넥션을 생성하는데 필요한 기간을 감소시킨다.
	...
	2. Connection Pool을 초기화
	   ...
	   a. DB Initial 클래스작성(DBCPInit.java 서블릿프로그램)
	   b. web.xml에 Connection Pool관련 태그 설정(myPoolName)
	
	3. Connectino Pool에서 Connection을 사용하기

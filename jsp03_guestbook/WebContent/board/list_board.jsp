...
<body style="background-color: silver;">
	<div class="container" align="center">
      ...
      <form action="b_write_message.jsp" method="post" class="form-line">
      ...
      <table class="table table-hover">
		...
		<tbody>
			<c:forEach var="message" items="${ view_data.messageList }">
	      		<tr>
	      			<td><a href="b_update_form.jsp?id=${ message.id }&cp=${ view_data.currentPage }">${ message.id }</a></td>
	      			<td>${ message.guestName }</td>
	      			<td>${ message.password }</td>
	      			<td>${ message.message }</td>
	      			<td><a href="b_delete_form.jsp?id=${ message.id }&cp=${ view_data.currentPage }"><i class="fas fa-trash-alt"></i></a></td>
	      		</tr>
      		</c:forEach>
		</tbody>
      </table>
	...
	<div class="container">
		<ul class="pagination justify-content-center">
			...
			<c:if test="${ startPage != 1 }">
				<li class="page-item"><a href="list_board.jsp?page=1" class="page-link"><i class="fas fa-fast-backward"></i></a></li>
				<li class="page-item"><a href="list_board.jsp?page=${ startPage - 10 }" class="page-link"><i class="fas fa-backward"></i></a></li>
			</c:if>
		
			<c:forEach var="page_num" begin="${ startPage }" end="${ endPage }">
				<li class="page-item"><a href="list_board.jsp?page=${ page_num }" class="page-link">${ page_num }</a></li>
			</c:forEach>
			
			<c:if test="${ endPage < view_data.pageTotalCount }">
				<li class="page-item"><a href="list_board.jsp?page=${ endPage + 1 }" class="page-link"><i class="fas fa-forward"></i></a></li>
				<li class="page-item"><a href="list_board.jsp?page=${ view_data.getPageTotalCount() }" class="page-link"><i class="fas fa-fast-forward"></i></a></li>
			</c:if>
		</ul>
	</div>
</body>

...
<!-- 
	...
	I. Cookie의 도메인
	   ...
	   하지만, 동일 도메인을 사용하는 모든 서버에 쿠키를 전송해야 할 경우 예를 들어
	   www.google.com에서 생성된 쿠키를 mail.google.com 또는 기타 동일 도메인에 포함된 서버에
	   전송할 경우가 있다.
	...
	K. 쿠키의 유효기간
	   ...
	   쿠키의 유효기간을 갖는다. 쿠키의 유효기간을 지정하지 않으면 웹브라우저를 종료할 때 쿠키를
	   함께 삭제한다. 웹브라우저 종료후 다시 웹브라우저를 실행하면 삭제한 쿠키는 서버에 전송되지
	   않는다. 쿠키의 유효기간을 정해 놓으면 유효기간동안 쿠키가 존재하고 웹브라우저가 종료되도
	   유효기간이 지나지 않았다면 그 쿠키는 삭제되지 않는다.
